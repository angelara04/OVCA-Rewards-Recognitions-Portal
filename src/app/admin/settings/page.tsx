...
export default function PortalSettingsPage() {
  ...
  const getEvaluationStatus = (dates: {
    start: string;
    end: string;
  }): EvaluationStatus => {
  ...
  const handleSaveAll = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      (nomDates.start && nomDates.end && nomDates.start >= nomDates.end) ||
        ...
        <div className="bg-white border border-indigo-100 rounded-xl shadow-sm overflow-hidden mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-indigo-50">
            <StatusMonitorItem
              ...
              status={nomEvalStatus}
            />
            <StatusMonitorItem
              label="Committee Evaluation Phase"
              status={scoreEvalStatus}
            />
          </div>
        </div>
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="bg-white p-6 border-b border-gray-100 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className={`px-2 py-1 rounded text-xs font-bold border ${badgeClass}`}
          >
            {status}
          </span>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 relative">
        ...
        <div>
          ...
          <input
            ...
            disabled={isReadOnly}
            className="w-full border border-gray-300 rounded-lg p-2.5 outline-none disabled:bg-gray-100"
          />
        </div>
        <div>
          <label className="block text-sm font-bold text-gray-700 mb-1">
...
function StatusMonitorItem({
  ...
  switch (status) {
    case "NO_SCHEDULE":
      ...
      desc = "Waiting for configuration.";
      break;
    case "NOT_STARTED":
      color = "text-yellow-600";
      icon = <Clock size={20} />;
      desc = "Scheduled but not yet active.";
      break;
    case "ONGOING":
      color = "text-green-600";
      icon = <CheckCircle size={20} />;
      desc = "Currently active and accessible.";
      break;
    case "COMPLETED":
      color = "text-red-600";
      icon = <Lock size={20} />;
      desc = "Period has ended.";
      break;
  }
