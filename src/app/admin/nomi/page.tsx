...
export default function NominationForm() {
  ...
  function startEditing(nom: any) {
    ...
    (
      document.querySelector("input[name='nominee_name']") as HTMLInputElement
    ).value = nom.nominee_name;
    (
      document.querySelector("input[name='position']") as HTMLInputElement
    ).value = nom.position;
    (document.querySelector("input[name='unit']") as HTMLInputElement).value =
      nom.unit;
    (
      document.querySelector(
        "input[name='length_of_service']"
      ) as HTMLInputElement
    ).value = nom.length_of_service;
    (
      document.querySelector(
        "textarea[name='achievements']"
      ) as HTMLTextAreaElement
    ).value = nom.achievements;
  ...
  // 2. New Handle Delete Function (prompt via banner)
  function handleDeleteDraft(id: string) {
    setPendingDeleteId(id);
    setBanner({
      title: "Confirm Deletion",
      message:
        "Do you want to delete this draft? This includes deleting all attached files. This action cannot be undone.",
      variant: "warning",
      duration: 600000,
    });
  }
  ...
  async function confirmDeleteDraft() {
    ...
    try {
      ...
      if (res.success) {
        ...
        setBanner({
          title: "Error",
          message: res.message || "Error deleting draft",
          variant: "error",
          duration: 4000,
        });
      }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {pendingDeleteId && (
        <ConfirmModal
          action="delete"
          onCancelAction={cancelDeleteDraft}
          onConfirmAction={confirmDeleteDraft}
        />
      )}
        ...
        {nominations.map((nom) => (
          ...
          >
            ...
            <p className="mt-6 text-gray-700">
              <strong>Category:</strong> {nom.category}
              <br />
              <strong>Position:</strong> {nom.position}
              <br />
