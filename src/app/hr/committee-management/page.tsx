...
export default function CommitteeManagementPage() {
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const [committees, nominators] = await Promise.all([getCommitteeMembers(), getNominators()]);
      ...
      const committeeRows: Employee[] = committees.map((c) => ({
        ...
        email: c.email,
        dateRegistered: formatDate(c.created_at),
        isCommittee: true,
      }));
      ...
      const nominatorRows: Employee[] = nominators
        .map((n) => ({
          ...
          displayId: n.id,
          name: n.name || "-",
          role: "Nominator" as const,
          department: n.department || "-",
          email: n.email,
          dateRegistered: formatDate(n.created_at),
          isCommittee: false,
        }))
  ...
  // explicit typing so key is a TabKey (fixes the second TS error)
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      base = base.filter((d) =>
        [d.name, d.email, d.department, d.displayId]
          .join(" ")
          .toLowerCase()
  ...
  async function handlePromote(userKeyId: string) {
    ...
    setData((prev) => prev.map((d) => (d.keyId === userKeyId ? { ...d, role: "Committee", isCommittee: true } : d)));
  }

  async function handleRemove(userKeyId: string) {
    const res = await removeFromCommittee(userKeyId);
    if (!res.success) return alert(res.message);
    setData((prev) => prev.map((d) => (d.keyId === userKeyId ? { ...d, role: "Nominator", isCommittee: false } : d)));
  }
  ...
  return (
    <Section width="w-full" height="min-h-screen" alignment="items-center p-10">
      ...
      <div className="max-w-6xl w-full bg-[var(--white)] border border-[var(--outline-grey)] border-t-0 rounded-b-xl shadow-sm px-6 py-6 min-h-[75vh] flex flex-col content-area relative -mt-[8px]">
