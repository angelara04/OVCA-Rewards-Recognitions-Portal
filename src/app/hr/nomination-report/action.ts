...
export async function getNominationReport() {
  const supabase = await createClient();
  const { data: auth } = await supabase.auth.getUser();
  if (!auth?.user) return [];
  ...
  const { data: nominations, error } = await supabase
    .from("nominations")
    .select(`id, nominee_name, category, reviews(total_score, reviewer_id)`)
    .eq("status", "completed");

  if (error || !nominations) return [];

  return nominations.map(nom => {
    const reviews = nom.reviews || [];
    const committeeScore = reviews.find(r => r.reviewer_id === myUserId)?.total_score || 0;
